...
class Solution:
    ...
    def create_excel_report_template(self, title_cells, subtitle_cells, column_widths):
        ...
        ws = wb.create_sheet("Program Delivery by School") 
        ...
        columns = ['School DBN', 'Primary Program Type',
                'Fully Receiving', 'Percent Fully Receiving', 'Partially Receiving', 'Percent Partially Receiving', 'Not Receiving', 'Percent Not Receiving']
    ...
    def connect_to_database(self):
        ...
        cursor.execute("EXEC [dbo].[USPCCTriannualReportPSSchoolLevel]")
        return cursor
    ...
    def write_data_to_excel(self, ws, data, start_row):
        ...
        for row in ws['C3':'H'+str(self.lastrow)]:  # Adjusted end row here
            for cell in row:
                if cell.value is not None:  # Ensure there is a value in the cell
                    cell.value = str(cell.value) + ''  # Prepend space to the value
        ...
        for row in ws['A'+str(self.lastrow): 'H'+str(self.lastrow)]:  # Adjusted end row here
            for cell in row:
                cell.border = black_boarder_all
                cell.font = Font(bold=True, size=12)
    ...
    def main_Program_Delivery_by_School(self):
        title_cells = [
            {"cell": "A1", "value": self.date +" Number & Percentage of Students Receiving Recommended Special Education Programs by Program Type", "merge_cells": "A1:H1"},
...
if __name__ == "__main__":
        Tab1_4 = Solution()
        Tab1_4.main_Program_Delivery_by_School()   
