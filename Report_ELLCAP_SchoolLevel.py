...
class Solution:
    ...
    def create_excel_report_template(self, title_cells, subtitle_cells, column_widths):
        ...
        self.format_header(ws, 'A5', "School Level", columns, column_letters, 60, header_fill_color, column_fill_color,  black_boarder_all_medium, header_font)
    ...
    def fetch_data_by_SchoolLevel(self,cursor):
        query_bySchoolLevel = '''
        Select 
        a.LocationCategoryDescription
        ...
        ,ReceivingD75
        FROM #Register as a
        left join( select LocationCategoryDescription
                    ,count(studentid) as 'TotalBIL'
                    FROM #Register
                    where [BilingualProgramRec] is not null 
                    group by LocationCategoryDescription) as z on a.LocationCategoryDescription = z.LocationCategoryDescription
        left join( select LocationCategoryDescription
                    ,count(studentid) as 'BilICT'
                    FROM #Register
                    where [BilingualProgramRec] = 'Integrated Co-Teaching Services'
                    and AdminDistrict <> 75
                    group by LocationCategoryDescription) as b on a.LocationCategoryDescription = b.LocationCategoryDescription
        left join( select LocationCategoryDescription
                    ,count(studentid) as 'BilSC'
                    FROM #Register
                    where [BilingualProgramRec] = 'Special Class'
                    and AdminDistrict <> 75
                    group by LocationCategoryDescription) as c on a.LocationCategoryDescription = c.LocationCategoryDescription
        left join( select LocationCategoryDescription
                    ,count(studentid) as 'BilMulti'
                    FROM #Register 
                    where [BilingualProgramRec]= 'Multiple Programs'
                    and AdminDistrict <> 75
                    group by LocationCategoryDescription) as d on a.LocationCategoryDescription = d.LocationCategoryDescription
        left join( select LocationCategoryDescription
                    ,count(studentid) as 'D75'
                    FROM #Register 
                    where [BilingualProgramRec] is not null
                    and AdminDistrict = 75
                    group by LocationCategoryDescription) as e on a.LocationCategoryDescription = e.LocationCategoryDescription
        left join( select LocationCategoryDescription
                    ,count(studentid) as 'TotalReceiving'
                    FROM #Register
                    where [BilingualProgramRec] is not null 
                    and ReceivingBilingual = 'FULLY RECEIVING'
                    group by LocationCategoryDescription) as Y on a.LocationCategoryDescription = Y.LocationCategoryDescription
        left join( select LocationCategoryDescription
                    ...
                    and AdminDistrict <> 75
                    and ReceivingBilingual = 'FULLY RECEIVING'
                    group by LocationCategoryDescription) as f on a.LocationCategoryDescription = f.LocationCategoryDescription
        left join( select LocationCategoryDescription
                    ...
                    and AdminDistrict <> 75
                    and ReceivingBilingual = 'FULLY RECEIVING'
                    group by LocationCategoryDescription) as g on a.LocationCategoryDescription = g.LocationCategoryDescription
        left join( select LocationCategoryDescription
                    ...
                    and AdminDistrict <> 75
                    and ReceivingBilingual = 'FULLY RECEIVING'
                    group by LocationCategoryDescription) as h on a.LocationCategoryDescription = h.LocationCategoryDescription
        left join( select LocationCategoryDescription
                    ...
                    and AdminDistrict = 75
                    and ReceivingBilingual = 'FULLY RECEIVING'
                    group by LocationCategoryDescription) as i on a.LocationCategoryDescription = i.LocationCategoryDescription
        left join( select LocationCategoryDescription
                    ,count(studentid) as 'BilSETSS'
                    FROM #Register
                    where [BilingualProgramRec] = 'SETSS'
                    and AdminDistrict <> 75
                    group by LocationCategoryDescription) as j on a.LocationCategoryDescription = j.LocationCategoryDescription
        left join( select LocationCategoryDescription
                    ...
                    and AdminDistrict <> 75
                    and ReceivingBilingual = 'FULLY RECEIVING'
                    group by LocationCategoryDescription) as k on a.LocationCategoryDescription = k.LocationCategoryDescription
        group by 
        a.LocationCategoryDescription
        ,TotalBIL
        ,BilICT
        ...
        ,ReceivingD75
        '''
        cursor.execute(query_bySchoolLevel)
    ...
    def write_data_to_excel(self, ws, data, start_row):
        ...
        for row in ws.iter_rows(min_col=1, max_col=ws.max_column, min_row=14, max_row=14):
            for cell in row:
                cell.fill = fill
        # add border to the last row
        for cell in ws['A14:B14'][0]:  # Only one row, so it's safe to use [0]
            cell.border = black_border_up_down
        for cell in ws['C14:N14'][0]:  # Only one row, so it's safe to use [0]
    ...
    def Report_Grade(self):
        ...
        results_bytab1 = self.fetch_data_by_SchoolLevel(cursor)
