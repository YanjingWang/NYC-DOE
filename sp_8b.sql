USE [SEO_REPORTING]
GO
/****** Object:  StoredProcedure [dev].[USPCCAnnaulReport1to4]    Script Date: 11/30/2023 1:33:59 PM ******/
SET ANSI_NULLS ON
GO
...
ALTER procedure [dev].[USPCCAnnaulReport1to4]
...
Pratap			11/02/2023			Initial version created this SQL script based on SAS code for Reports 1-4
************************************************************************************************************************/
BEGIN
	...
	declare @prevYear_YY char(2)
	set @prevYear_YY  = right(CONVERT(char(10), getdate(), 101), 2) -1 

	declare @old_Referraldate varchar(10)
	set @old_Referraldate = '07-01-20'+@prevYear_YY

	declare @new_Referraldate varchar(10)
	set @new_Referraldate = '07-01-20'+@currYear_YY
...
' left join SEO_Mart.snap.'+@tableNameINTStudentDemographics_0630 +' c on a.studentid=c.studentid'+ 
--' WHERE referraldate < '+'''07-01-2023'''+' AND referraldate >= '+'''07-01-2022'''+
' WHERE referraldate < '+ ''''+@new_Referraldate+''''+' AND referraldate >= '+''''+@old_Referraldate+''''+
...
+ case when @sortBy = '' then  @groupBy +' as sort ' 
	when  @i = 8 then ' case when '+@groupBy + ' = ' +'''Y'''+' then '+'''Yes'''+' else '+'''No'''+'  end as TempResFlag'
	else  @groupBy  end   
+'	,FORMAT(sum(STUDENTS_WITH_REF), ''#,##0'') as c1 '+
'    ,FORMAT(sum(CLOSED_WITHOUT_IEP), ''#,##0'') as c2 '+
'	,FORMAT(sum(INELIGIBLE_LESS_60), ''#,##0'') as c3 '+
...
'	,FORMAT(sum(TOTAL_OPEN), ''#,##0'') as c11 '+
' FROM ##Report_Final '+
case when @i =8 then ' where TempResFlag in ('+'''Y'''+', '+'''N'''+') ' 
	 else ' ' end 
