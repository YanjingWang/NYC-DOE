...
class Solution:
    ...
    def create_excel_report_template(self, title_cells, subtitle_cells, column_widths):
        ...
        ws = wb.create_sheet("Report 11 = 3Yr Reevaluations")
    ...
    def connect_to_database(self):
        ...
        cursor = conn.cursor()
        params = ('CC_ThreeYearReevalsR11_SY23_fix', 'INT_StudentDemographics_063023')
        cursor.execute("EXEC [dev].[USPCCAnnaulReport11] @tableNameCCThreeYearReevalsR11=?,  @tableNameINTStudentDemographics_0630=?", params)
        return cursor
    # Fetch data for "Report 8b = IEP Service Recs by Race"
    def fetch_data_by_race(self,cursor):
        query_byRace = '''
        select EthnicityGroupCC, c1,c2,c3 from (  select * from  (  Select   EthnicityGroupCC_sort as sort , EthnicityGroupCC 	 ,FORMAT(Count(ComplianceStatus), '#,##0') as c1         ,FORMAT(sum(Conduct_on_time), '#,##0') as c2        ,
        FORMAT(sum(Not_conducT_on_t), '#,##0') as c3  FROM ##Report_Final   group by EthnicityGroupCC, EthnicityGroupCC_sort ) a  union all  select * from ##TotalRow_Sort ) a order by sort 
    ...
    def fetch_data_by_district(self,cursor):
        query_byDistrict = '''
        select * from  (  Select    ReportingDistrict as sort  	 ,FORMAT(Count(ComplianceStatus), '#,##0') as c1         ,FORMAT(sum(Conduct_on_time), '#,##0') as c2        ,
        FORMAT(sum(Not_conducT_on_t), '#,##0') as c3  FROM ##Report_Final   group by ReportingDistrict  ) a  union all  select * from ##TotalRow  order by sort 
    ...
    def fetch_data_by_mealstatus(self,cursor):
        query_byMealStatus = '''
        select * from  (  Select    MealStatusGrouping as sort  	 ,FORMAT(Count(ComplianceStatus), '#,##0') as c1         ,FORMAT(sum(Conduct_on_time), '#,##0') as c2        ,
        FORMAT(sum(Not_conducT_on_t), '#,##0') as c3  FROM ##Report_Final   group by MealStatusGrouping  ) a  union all  select * from ##TotalRow  order by sort 
    ...
    def fetch_data_by_gender(self,cursor):
        query_byGender = '''
        select * from  (  Select    GENDER as sort  	 ,FORMAT(Count(ComplianceStatus), '#,##0') as c1         ,FORMAT(sum(Conduct_on_time), '#,##0') as c2        ,
        FORMAT(sum(Not_conducT_on_t), '#,##0') as c3  FROM ##Report_Final   group by GENDER  ) a  union all  select * from ##TotalRow  order by sort 
    ...
    def fetch_data_by_ellstatus(self,cursor):
        query_byELLStatus = '''
        select * from  (  Select    ELLStatus as sort  	 ,FORMAT(Count(ComplianceStatus), '#,##0') as c1         ,FORMAT(sum(Conduct_on_time), '#,##0') as c2        ,
        FORMAT(sum(Not_conducT_on_t), '#,##0') as c3  FROM ##Report_Final   group by ELLStatus  ) a  union all  select * from ##TotalRow  order by sort 
    ...
    def fetch_data_by_language(self,cursor):
        query_byLanguage = '''
        select OutcomeLanguageCC, c1,c2,c3 from (  select * from  (  Select   LanguageOfInstruction_Sort as sort , OutcomeLanguageCC 	 ,FORMAT(Count(ComplianceStatus), '#,##0') as c1         ,FORMAT(sum(Conduct_on_time), '#,##0') as c2        ,
        FORMAT(sum(Not_conducT_on_t), '#,##0') as c3  FROM ##Report_Final   group by OutcomeLanguageCC, LanguageOfInstruction_Sort ) a  union all  select * from ##TotalRow_Sort ) a order by sort 
    ...
    def fetch_data_by_gradelevel(self,cursor):
        query_byGradeLevel = '''
        select GradeLevel, c1,c2,c3 from (  select * from  (  Select   GradeLevel_Sort as sort , GradeLevel 	 ,FORMAT(Count(ComplianceStatus), '#,##0') as c1         ,FORMAT(sum(Conduct_on_time), '#,##0') as c2        ,
        FORMAT(sum(Not_conducT_on_t), '#,##0') as c3  FROM ##Report_Final   group by GradeLevel, GradeLevel_Sort ) a  union all  select * from ##TotalRow_Sort ) a order by sort 
        '''
    ...
    def fetch_data_by_tempResFlag(self,cursor):
        query_byTempResFlag = '''
        select TempResFlag, c1,c2,c3 from (  select * from  (  Select   TempResFlagSort as sort ,  case when TempResFlag = 'Y' then 'Yes' else 'No'  end as TempResFlag 	 ,FORMAT(Count(ComplianceStatus), '#,##0') as c1         ,FORMAT(sum(Conduct_on_time), '#,##0') as c2        ,
        FORMAT(sum(Not_conducT_on_t), '#,##0') as c3  FROM ##Report_Final  where TempResFlag in ('Y', 'N')  group by TempResFlag, TempResFlagSort ) a  union all  select * from ##TotalRow_Sort ) a order by sort  
        '''
    ...
    def fetch_data_by_fosterCareStatus(self,cursor):
        ...
        select FosterCareFlag, c1,c2,c3 from (  select * from  (  Select   FosterCareFlagSort as sort , FosterCareFlag 	 ,FORMAT(Count(ComplianceStatus), '#,##0') as c1         ,FORMAT(sum(Conduct_on_time), '#,##0') as c2        ,
        FORMAT(sum(Not_conducT_on_t), '#,##0') as c3  FROM ##Report_Final   group by FosterCareFlag, FosterCareFlagSort ) a  union all  select * from ##TotalRow_Sort ) a order by sort 
        '''
...
if __name__ == "__main__":
        Tab1 = Solution()
        Tab1.Report_11_3Yr_Reevaluations()                                                                  