...
class Solution:
    ...
    def create_excel_report_template(self, title_cells, subtitle_cells, column_widths):
        ...
        self.format_header(ws, 'A5', "Grade Level", columns, column_letters, 60, header_fill_color, column_fill_color,  black_boarder_all_medium, header_font)
    ...
    def fetch_data_by_Grade(self,cursor):
        query_byGrade = '''
        --Grade Level
        ...
        left join( select GradeLevel
                    ...
                    and AdminDistrict <> 75
                    and ReceivingBilingual = 'FULLY RECEIVING'
                    group by GradeLevel) as f on a.GradeLevel = f.GradeLevel
        left join( select GradeLevel
                    ...
                    and AdminDistrict <> 75
                    and ReceivingBilingual = 'FULLY RECEIVING'
                    group by GradeLevel) as g on a.GradeLevel = g.GradeLevel
        left join( select GradeLevel
                    ...
                    and AdminDistrict <> 75
                    and ReceivingBilingual = 'FULLY RECEIVING'
                    group by GradeLevel) as h on a.GradeLevel = h.GradeLevel
        left join( select GradeLevel
                    ...
                    and AdminDistrict = 75
                    and ReceivingBilingual = 'FULLY RECEIVING'
                    group by GradeLevel) as i on a.GradeLevel = i.GradeLevel
        left join( select GradeLevel
                    ,count(studentid) as 'BilSETSS'
                    FROM #Register
                    where [BilingualProgramRec] = 'SETSS'
                    and AdminDistrict <> 75
                    group by GradeLevel) as j on a.GradeLevel = j.GradeLevel
        left join( select GradeLevel
                    ...
                    and AdminDistrict <> 75
                    and ReceivingBilingual = 'FULLY RECEIVING'
                    group by GradeLevel) as k on a.GradeLevel = k.GradeLevel
        group by 
        a.GradeLevel
        ,TotalBIL
        ,BilICT
    ...
    def write_data_to_excel(self, ws, data, start_row):
        ...
        for col in ['C', 'I','N']:  # Columns F and L are the 'Total' columns
            for row_num in range(start_row, ws.max_row + 1):
                ws[col + str(row_num)].fill = fill
        
        # Fill 'Total' row with the specified color
        for row in ws.iter_rows(min_col=1, max_col=ws.max_column, min_row=19, max_row=19):
            for cell in row:
                cell.fill = fill
        # add border to the last row
        for cell in ws['A19:B19'][0]:  # Only one row, so it's safe to use [0]
            cell.border = black_border_up_down
        for cell in ws['C19:N19'][0]:  # Only one row, so it's safe to use [0]
    ...
    def Report_Grade(self):
        ...
        results_bytab1 = self.fetch_data_by_Grade(cursor)
