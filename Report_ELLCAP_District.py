...
class Solution:
    ...
    def create_excel_report_template(self, title_cells, subtitle_cells, column_widths):
        ...
        for cell_info in subtitle_cells:
            ...
            cell.value = cell_info["value"]
            ws.merge_cells(cell_info["merge_cells"])  # Merge cells outside the loop
            cell.border = black_border_medium #C4:H4 and I4:N4
    ...
    def fetch_data_by_District(self,cursor):
        ...
        ,ReceivingD75
        FROM #Register as a
        left join( select  [AdminDistrict]
                    ,count(studentid) as 'TotalBIL'
                    FROM #Register
                    where [BilingualProgramRec] is not null 
                    group by   [AdminDistrict]) as z on a. [AdminDistrict] = z. [AdminDistrict]
        left join( select  [AdminDistrict]
                    ,count(studentid) as 'BilICT'
                    FROM #Register
                    where [BilingualProgramRec] = 'Integrated Co-Teaching Services'
                    and AdminDistrict <> 75
                    group by  [AdminDistrict]) as b on a. [AdminDistrict] = b. [AdminDistrict]
        left join( select  [AdminDistrict]
                    ,count(studentid) as 'BilSC'
                    FROM #Register
                    where [BilingualProgramRec] = 'Special Class'
                    and AdminDistrict <> 75
                    group by  [AdminDistrict]) as c on a.[AdminDistrict] = c. [AdminDistrict]
        left join( select  [AdminDistrict]
                    ,count(studentid) as 'BilMulti'
                    FROM #Register 
                    where [BilingualProgramRec]= 'Multiple Programs'
                    and AdminDistrict <> 75
                    group by  [AdminDistrict]) as d on a.[AdminDistrict] = d.[AdminDistrict]
        left join( select AdminDistrict
                    ,count(studentid) as 'D75'
                    FROM #Register 
                    where [BilingualProgramRec] is not null
                    and AdminDistrict = 75
                    group by AdminDistrict) as e on a.AdminDistrict = e.AdminDistrict
        left join( select AdminDistrict
                    ,count(studentid) as 'TotalReceiving'
                    FROM #Register
                    where [BilingualProgramRec] is not null 
                    and ReceivingBilingual = 'FULLY RECEIVING'
                    group by AdminDistrict) as Y on a.AdminDistrict = Y.AdminDistrict
        left join( select AdminDistrict
                    ...
                    and AdminDistrict <> 75
                    and ReceivingBilingual = 'FULLY RECEIVING'
                    group by AdminDistrict) as f on a.AdminDistrict = f.AdminDistrict
        left join( select AdminDistrict
                    ...
                    and AdminDistrict <> 75
                    and ReceivingBilingual = 'FULLY RECEIVING'
                    group by AdminDistrict) as g on a.AdminDistrict = g.AdminDistrict
        left join( select AdminDistrict
                    ...
                    and AdminDistrict <> 75
                    and ReceivingBilingual = 'FULLY RECEIVING'
                    group by AdminDistrict) as h on a.AdminDistrict = h.AdminDistrict
        left join( select AdminDistrict
                    ...
                    and AdminDistrict = 75
                    and ReceivingBilingual = 'FULLY RECEIVING'
                    group by AdminDistrict) as i on a.AdminDistrict = i.AdminDistrict
        left join( select AdminDistrict
                    ,count(studentid) as 'BilSETSS'
                    FROM #Register
                    where [BilingualProgramRec] = 'SETSS'
                    and AdminDistrict <> 75
                    group by AdminDistrict) as j on a.AdminDistrict = j.AdminDistrict
        left join( select AdminDistrict
                    ...
                    and AdminDistrict <> 75
                    and ReceivingBilingual = 'FULLY RECEIVING'
                    group by AdminDistrict) as k on a.AdminDistrict = k.AdminDistrict
        group by 
        a.AdminDistrict
        ,TotalBIL
        ,BilICT
    ...
    def write_data_to_excel(self, ws, data, start_row):
        ...
        for row in ws['A6':'A'+str(self.lastrow)]:
            for cell in row:
                ...
                cell.alignment = openpyxl.styles.Alignment(horizontal='center', vertical='center')  # Center align the text
        for row in ws['B6':'B'+str(self.lastrow)]:
            for cell in row:
                if cell.value is not None:  # Ensure there is a value in the cell
                    cell.alignment = openpyxl.styles.Alignment(horizontal='right', vertical='center')  # Center align the text
                
        for row in ws['C6':'N'+str(self.lastrow)]:
            for cell in row:
    ...
    def Report_District(self):
        ...
        results_bytab1 = self.fetch_data_by_District(cursor)
